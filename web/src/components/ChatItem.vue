    ...
    <template v-for="item in props.chatData" :key="item.id">
      <div class="my-4" v-if="item.type === 'me' || item.type === 'your'">
        <div :class="[
          ...
          <!-- 头像 -->
          <div :class="['avatar', item.type === 'your' ? 'mr-2' : 'ml-2']" @click="handleClickAvatar(item)">
          ...
          <div :class="['user', item.type === 'me' ? 'flex flex-col' : '']">
            <span class="text-base-content/100" :class="item.type === 'me' ? 'self-end' : ''">{{ item.name }}</span>
            <div class="p-2 mt-2 rounded-t max-w-192 text-base-content/100"
